                   ...
                   BITSET64 c, BITSET64 u, int noPlies, int o, int minscore, int maxscore)
{
  ...
  // creating a fork which will create a  child process
  int pid = fork();
  // if pid is 0 then its a child process
  if (pid == 0)
  {
    // child is the source which spawns each move on a separate core.
    // child process will spawn all the children and further childeren that will spawn the moves
    for (int i = 0; i < noOfMoves; i++)
    {
      // waiting for the process to become available
      multiprocessor_wait(processorAvailable);
      // definding the move score veriable
      int move_score;
      // if the process is a child then run this
      if (fork() == 0)
      {
        // the child searches to move i
        // using alpha beta alogorithm to search the move for i
        move_score = alphaBeta(listOfMoves[i], c, u, noPlies, o, minscore, maxscore);

        // passing move_score, i ,positions_explored back via mailbox using mailbox send
        mailbox_send(barrier, move_score, i, noOfMoves);

        multiprocessor_signal(processorAvailable); // core is now aviable
        exit(0);                                   // kill the process
      }
    }
    exit(0); // kill the process
  }
  else // it must be a parent process then
  {
    ...
    // run as many times as there are moves
    for (i = 0; i < noOfMoves; i++)
    {
      // printing in the consol that the program is waiitng for a result this is becasue search can take a while to complete
      printf("parent waiting for a result\n");
      // receving the veriables using the refrence
      ...
      *totalExplored += positions_explored; /* add count to the running total.  */
      // if the move score is greator than the current best, if we have a better move
      if (move_score > best)
      {
        // saving the move score to best veriable
        best = move_score;
        // remmeber the move
